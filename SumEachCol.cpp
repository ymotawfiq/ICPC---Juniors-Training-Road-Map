...
void SumOfEachColumnInMatrix(int arr[3][3], int sum[3], int rows, int col)
{
	for(int i=0; i<rows; i++)
	{
		int rowSum = 0;
...
void SumOfEachColumnInMatrix2(int arr[3][3], int rows, int col)
{
	for(int i=0; i<rows; i++)
	{
		int colSum = ColSum(arr, rows, i);
...
int main()
{
	...
	cout<<"\nThe following is sum of each col\n";
	SumOfEachColumnInMatrix(arr, sum, rows, col);
	PrintArray(sum, rows);
	cout<<"\n";
	SumOfEachColumnInMatrix2(arr, rows, col);
}