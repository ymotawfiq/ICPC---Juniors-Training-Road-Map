...
int CountDaysBetweenPeriods(stDate period1, stDate period2)
{
	int c = 0;
	while(CompareDates(period1, period2)<0)
	{
		c++;
		period1 = IncreaseDateByOne(period1);
	}
	return c;
}
...
{
	int c = 0;
	if(!IsOverlap2(period1Start, period1End, period2Start, period2End))return c;
	int period1Length = CountDaysBetweenPeriods(period1Start, period1End);
	int period2Length = CountDaysBetweenPeriods(period2Start, period2End);
	if(period1Length<period2Length)
	{
		while(IsDateInPeriod(period1Start, period1End, period2Start))
		{
			c++;
	...
	{
		while(IsDateInPeriod(period2Start, period2End, period1Start))
		{
			c++;
			period2Start = IncreaseDateByOne(period2Start);
		}
	}
	return c;
}
...
int main()
{
	...
	cout<<countOverLap(period1Start, period1End, period2Start, period2End)<<endl;
